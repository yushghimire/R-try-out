	...
	if(empty($input))
{
	...
	$sql = "SELECT * FROM customer NATURAL JOIN tbl_users  where userName like '%$input%'";
	...
	if (mysqli_num_rows($result) > 0) 
	{
		...
		{ 
			echo '<a  class="list-group-item list-group-item-action list-group-item-info justify-content-between" >'. $row["userName"]. '--- Cluster: '.$row["cluster"].'<br>
			Email: '.$row["email"];
			}
}
